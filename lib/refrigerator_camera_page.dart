...
'''
      ...
      final rawExpireDate = result['expireDate'];
      final String? expireDate =
          rawExpireDate == null ? null : rawExpireDate.toString();

      final rawDays = result['recommendedStorageDays'];
  ...
  Future<void> _registerItem() async {
    ...
    await FirebaseFirestore.instance.collection('refrigeratorItems').add({
      ...
      'expireSource': _foodType == 'PACKAGED' ? _expireSource : null,
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                    '추천 보관일: ${_recommendedStorageDays ?? 7}일'),
                                const SizedBox(height: 4),
                                Text(
                                  '추천 수거일: ${_recommendedExpireDate ?? '미확인'}',
                                ),
                              ],
                            ),
                          ),
