...
class HomePage extends StatelessWidget {
  ...
  Future<void> _checkFridgeExpiryNotifications(String uid) async {
    ...
    for (final doc in items.docs) {
      ...
      final hoursLeft = expireDate.difference(now).inHours;

      String? notificationType;
      String? message;

      if (hoursLeft <= 48 && hoursLeft > 24) {
        notificationType = 'FRIDGE_EXPIRE_48H';
        message = '$itemName의 추천 수거일까지 48시간 이내로 남았습니다.';
      } else if (hoursLeft <= 24 && hoursLeft >= 0) {
        notificationType = 'FRIDGE_EXPIRE_24H';
        message = '$itemName의 추천 수거일까지 24시간 이내로 남았습니다.';
      }
